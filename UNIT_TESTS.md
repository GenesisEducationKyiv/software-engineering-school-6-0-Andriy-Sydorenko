...
- **Mocks**: `go.uber.org/mock` (uber-go/mock, gomock's maintained
  ...
  `go.mod`), committed to the repo. CI does not run `go generate` —
  a stale mock surfaces as a compile error.
- **Runner**: stdlib `testing`. No third-party runner.

### Mocks live next to the interfaces they mock

Each interface-owning package has a `//go:generate` directive:
...
`<pkg>/mocks`. After changing any interface signature, run
`make generate`.
